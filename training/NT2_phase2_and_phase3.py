...
EXPERIMENT_CONFIGS = [
    ...
    # ========================================================================
    # Note: Original Phase 2 experiments (exp_id 17-19) were run without 
    # gradient accumulation. These should be re-run with gradient_accumulation_steps=8
    # in the future to match the effective batch size of 32 used in Phase 1.
    # 
    ...
    { # LoRA rank 8
        ...
        "lora_rank": 8,
        "batch_size": 4,
        "num_steps": 4000, # smaller run for subset data
    ...
    # ========================================================================
    # Phase 3: Learning Rate Comparison with Gradient Accumulation
    # ========================================================================
    # Best LoRA rank from Phase 2: 32
    # Using gradient accumulation to achieve effective batch size of 32
...
class NT2_FineTune(nn.Module):
    ...
    def __init__(self, base_model, classifier_type, num_layers, 
        ...
        if lora_rank is not None:
            ...
            lora_config = LoraConfig(
                ...
                lora_dropout=0.1,
                # Option 1: Conservative (Query + Value only) - Recommended to Start
                target_modules=["query", "value"],
                # # Option 2: Full Attention (Query + Key + Value)
                # target_modules = ["query", "key", "value"],
                # # Option 3: Attention + Output (Most Aggressive)
                # target_modules = ["query", "key", "value", "dense"],
                bias="none"
            )

            # Apply LoRA (this adds trainable LoRA parameters)
            ...
            print("="*80)
            print("LoRA Configuration Applied with Gradient Checkpointing:")
            self.bert.print_trainable_parameters()
            print("="*80)

        else:
            raise ValueError(
                "This code supports only LoRA fine-tuning. Please specify lora_rank in config. "
                "If not using LoRA, please use the NT2-phase1_and_unfreezeAll.py code."
            )
    ...
    def forward(self, input_ids, attention_mask=None):
        ...
        if self.embedding_strategy == "variant_position":
            # Variant is at nucleotide position 6000 (1-indexed) = 5999 (0-indexed)
            # After 6-mer tokenization: position 5999 // 6 = 999 (k-mer index)
            # After <cls> token: position 1000 in the tokenized sequence
            variant_position = 1000
            seq_len = embed.shape[1]  # ADD THIS LINE
...
def train_experiment(exp_config, train_path, val_path, output_dir, 
    ...
    try:
        ...
        # Get LoRA rank if specified
        lora_rank = exp_config.get('lora_rank', None) if exp_config['fine_tuning'] == 'lora' else None
        ...
        while global_step < num_steps:
            ...
            if accumulation_counter == gradient_accumulation_steps:
                ...
                scheduler.step()
                accumulation_counter = 0
                global_step += 1  # Only increment after actual update
            
            # # Log every 1000 steps
            # if global_step % 1000 == 0:
            
            # Log every 1000 steps (only after weight updates)
            if global_step % 1000 == 0 and global_step > 0 and accumulation_counter == 0:
        ...
        summary = {
            ...
            'effective_batch_size': batch_size * gradient_accumulation_steps  # ADD THIS
